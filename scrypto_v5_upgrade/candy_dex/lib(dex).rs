...
blueprint! {
    ...
    impl CandyDex {
        pub fn new(fee: Decimal) -> (ComponentAddress,Bucket) {
            ...
            let mut candy_dex: CandyDexComponent = Self {
            ...
            .instantiate();
            candy_dex.add_access_check(access_rules);
            
            (candy_dex.globalize(),badge_bucket)
        }
            ...
            fn tokenput_pri(
                ...
                if prc_in == dec!("0") { 
                    token_amnt = CandyDex::adjust_fee(self, token_amnt); 
                }
                
                // Call a method to calculate token output amount.
                let amount = CandyDex::token_sum(self, token_amnt, token_addr, addr_in, 1);
            ...
            fn tokenput_sec(
                ...
                if prc_in == dec!("0") {
                    let amount_in = CandyDex::token_sum(self, amnt, addr, token_addr, 2); 
            ...
            fn token_sum(
                ...
                let price_in: Decimal = CandyDex::tokenprice(self, addr_pri);
                let price_out: Decimal = CandyDex::tokenprice(self, addr_sec);
        ...
        pub fn unstock_token(
            ...
            // Display amounts
            CandyDex::info_amounts(*total_minted,*accrued_fee,*entry_fee,delta_fee,token_out_nbr);
            ...
            if token_out_nbr <= total_token {
                ...
                let price_in: Decimal = CandyDex::tokenprice(self, token_addr);
        ...
        pub fn stock_position(&mut self, meta_badge: Proof) {
            ...
            match self.badge_map.get(&meta_badge.resource_address()) {
                Some((a,b,c)) => { 
                    ...
                    // Display amounts
                    CandyDex::info_amounts(*total_minted,*accrued_fee,*a,delta_fee,token_out_nbr);
                },
                None => {
        ...
        pub fn get_token_buy_amount_bcsex(
            ...
            let price = CandyDex::tokenprice(self, token_addr);
